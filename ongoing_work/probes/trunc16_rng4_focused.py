"""rng=4 focused probe on B4 truncated 16-cell.

Same protocol as rect_tess_rng4_focused.py but for B4 (0,0,1,1)
truncated 16-cell, V=48 E=120.  Kernel-completeness audit reported
pipeline=3, full_rng2=7, gap=4.  Rectified tesseract (gap 4) gained
3 new zomeable shapes at rng=4; this probe tests whether the same
pattern holds for truncated 16-cell.
"""
...
OUT = Path('ongoing_work/truncated_16cell_rng4')
...
V, E = build_polytope('B4', (0, 0, 1, 1))
print(f'truncated 16-cell V={len(V)} E={len(E)}', flush=True)
...
t0 = time.time()
hits = search('truncated 16-cell', V, E, dirs4, verbose=False)
groups = group_by_shape(hits, V, E)
search_time = time.time() - t0
print(f'rng=4 raw fp = {len(groups)} ({search_time:.1f}s)', flush=True)

with open('ongoing_work/truncated_16cell_rng4_kernels.json', 'w') as f:
    json.dump(
        [{'fp_idx': i,
          'fp_hash': str(fp),
...
for fp_idx, (fp, examples) in enumerate(groups.items()):
    ...
    out_path = str(OUT / f'trunc_16c_fp{fp_idx:02d}_kernel_{n_list}.vZome')
    out_path = out_path.replace(' ', '').replace('[', '').replace(']', '')
    out_path = out_path.replace(',', '_')
    try:
        project_and_emit('truncated 16-cell', V, E, n_list, out_path,
                         verbose=False)
...
with open('ongoing_work/truncated_16cell_rng4.json', 'w') as f:
    json.dump({
...
print('\nSaved ongoing_work/truncated_16cell_rng4.json')
