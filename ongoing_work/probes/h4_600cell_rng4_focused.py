"""rng=4 focused probe on H4 600-cell.

Same protocol as the B4 rng=4 focused probes but for H4 (0,0,0,1)
600-cell, V=120 E=720.  This is the only tractable H4 polytope at
rng=4 in the current implementation (~19h wall projected).

The 600-cell rng=2 audit already showed gap=0 (1 master shape =
600cell_H4_to_H3.vZome recovered).  This probe asks the second
question: do larger Z[phi] kernel directions reveal *new* zomeable
shapes beyond rng=2?  By analogy with the B4 audit-flagged cases
(rectified tesseract gap=4 -> +3, truncated 16-cell gap=4 -> +3),
the answer for an audit-clean polytope is *expected* to be 0, but
this is the strongest H4 empirical test we can run.
"""
...
OUT = Path('ongoing_work/h4_600cell_rng4')
...
V, E = build_polytope('H4', (0, 0, 0, 1))
print(f'600-cell V={len(V)} E={len(E)}', flush=True)
...
t0 = time.time()
hits = search('600-cell', V, E, dirs4, verbose=False)
groups = group_by_shape(hits, V, E)
search_time = time.time() - t0
print(f'rng=4 raw fp = {len(groups)} ({search_time:.1f}s)', flush=True)

with open('ongoing_work/h4_600cell_rng4_kernels.json', 'w') as f:
    json.dump(
        [{'fp_idx': i,
          'fp_hash': str(fp),
...
for fp_idx, (fp, examples) in enumerate(groups.items()):
    ...
    out_path = str(OUT / f'h4_600c_fp{fp_idx:02d}_kernel_{n_list}.vZome')
    out_path = out_path.replace(' ', '').replace('[', '').replace(']', '')
    out_path = out_path.replace(',', '_')
    try:
        project_and_emit('600-cell', V, E, n_list, out_path,
                         verbose=False)
...
with open('ongoing_work/h4_600cell_rng4.json', 'w') as f:
    json.dump({
...
print('\nSaved ongoing_work/h4_600cell_rng4.json')
