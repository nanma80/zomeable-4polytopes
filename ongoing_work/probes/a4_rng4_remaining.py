"""rng=4 probe on remaining A4 Wythoff descendants.

Completes the A4 rng=4 sweep (cases 5-7 of rng4_descendants.py that
were never run because the script stopped to focus on rectified
tesseract).  A4 descendants up to rng=3 were stable; this verifies
they remain stable at rng=4 and confirms no new A4 shapes are missed.
"""
...
OUT = Path('ongoing_work/a4_rng4_remaining')
OUT.mkdir(exist_ok=True)
summary = []

for group, bm, name in cases:
    print(f'\n=== {name} {group} {bm} ===', flush=True)
...
with open('ongoing_work/a4_rng4_remaining.json', 'w') as f:
    json.dump(summary, f, indent=2, default=str)
print('\nSaved ongoing_work/a4_rng4_remaining.json')
