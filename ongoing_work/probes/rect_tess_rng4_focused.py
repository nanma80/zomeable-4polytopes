"""Focused rng=4 probe on rectified tesseract (B4 (0,1,0,0)).

Initial rng=4 sweep reported 7 raw fp -> 7 snap pass -> 6 Stage-B
unique -> 3 in existing rectified_tesseract corpus -> *3 NEW shapes
not in corpus*.  All 3 had V=32 E=96 with edge multiset
((1.0, 24), (1.47337, 24), (1.61803, 24), (1.7013, 24)) -- a 4-edge-
length structure absent from the existing rng=2 corpus (which had only
2-edge-length shapes).

This focused probe saves the snapped .vZome files permanently so we
can hand-inspect them and verify they are valid zomeable projections.
"""
...
OUT = Path('ongoing_work/rectified_tesseract_rng4')
...
V, E = build_polytope('B4', (0, 1, 0, 0))
print(f'rectified tesseract V={len(V)} E={len(E)}', flush=True)

# Pre-build corpus index now so search can be redone without re-indexing
...
for vz in Path('output/wythoff_sweep').rglob('*.vZome'):
    try:
        ...
        print(f'  skip {vz}: {type(exc).__name__}: {exc}', flush=True)
    ...
    if p.exists():
        for vz in p.glob('*.vZome'):
            try:
                ...
                print(f'  skip {vz}: {type(exc).__name__}: {exc}',
                      flush=True)
print(f'  {len(corpus_sigs)} unique sigs across '
      f'{sum(len(v) for v in corpus_sigs.values())} files', flush=True)

t0 = time.time()
hits = search('rectified tesseract', V, E, dirs4, verbose=False)
groups = group_by_shape(hits, V, E)
search_time = time.time() - t0
print(f'rng=4 raw fp = {len(groups)} ({search_time:.1f}s)', flush=True)

# Save canonical kernel directions immediately so a parse failure
# downstream doesn't cost us the search.
with open('ongoing_work/rectified_tesseract_rng4_kernels.json', 'w') as f:
    json.dump(
        [{'fp_idx': i,
          'fp_hash': str(fp),
        ...
        f, indent=2)
print('  saved kernels to rectified_tesseract_rng4_kernels.json',
      flush=True)


# Snap each fp and save permanently
records = []
for fp_idx, (fp, examples) in enumerate(groups.items()):
    n0, sig0, balls0 = examples[0]
    n_list = list(map(float, n0))
    out_path = str(OUT / f'rect_tess_fp{fp_idx:02d}_kernel_{n_list}.vZome')
    out_path = out_path.replace(' ', '').replace('[', '').replace(']', '')
    out_path = out_path.replace(',', '_')
    try:
        project_and_emit('rectified tesseract', V, E, n_list,
...
with open('ongoing_work/rectified_tesseract_rng4.json', 'w') as f:
    json.dump({
...
print('\nSaved ongoing_work/rectified_tesseract_rng4.json')
print('Output files:')
for p in OUT.glob('*.vZome'):
    print(f'  {p}')
