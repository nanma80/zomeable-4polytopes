...
def demicube_edges(vertices):
    edges = []
    for i, a in enumerate(vertices):
        for j in range(i + 1, len(vertices)):
            b = vertices[j]
            if sum(a[k] != b[k] for k in range(len(a))) == 2:
                edges.append((i, j))
    return edges
...
def project_vertex(signs, columns):
    ...
    for s, col in zip(signs, columns):
        coeff = GF(Fraction(s, 2))
        p = add(p, vscale(tuple(gf_from_pair(x) for x in col), coeff))
    return p


def projected_model_raw(columns, n: int):
    vertices = demicube_vertices(n)
    source_edges = demicube_edges(vertices)
    raw_points = []
    point_index = {}
    v_to_p = []
    for v in vertices:
        p = project_vertex(v, columns)
        k = vkey(p)
        if k not in point_index:
            point_index[k] = len(raw_points)
            raw_points.append(p)
    ...
    return raw_points, sorted(edge_set), collapsed, len(source_edges)
...
def audit_scale(raw_points, edges, collapsed, source_edge_count, scale, stop_on_bad: bool):
    points = [vscale(p, scale) for p in raw_points]
    oriented = []
...
def choose_scale(columns, n: int):
    raw_points, edges, collapsed, source_edge_count = projected_model_raw(columns, n)
    best = None
    for label, scale in scale_candidates():
        audit = audit_scale(raw_points, edges, collapsed, source_edge_count, scale, stop_on_bad=True)
        if audit["bad"]:
            continue
        ...
        if best is None or candidate[0] < best[0]:
            best = candidate
    if best is not None:
        _, label, scale, _audit = best
        # Recompute without short-circuiting so diagnostics are complete.
        audit = audit_scale(raw_points, edges, collapsed, source_edge_count, scale, stop_on_bad=False)
...
def default_symbol(n: int):
    known = {5: "1_21", 6: "1_31", 7: "1_41", 10: "1_71"}
    return known.get(n, f"{n}_demicube")
...
def main():
    ...
    prefix = args.prefix or default_symbol(args.n)
    manifest = []
    for idx, (sig, hit) in enumerate(sorted(data["hits"].items(), key=lambda item: (item[1]["N"], item[0])), start=1):
        ...
        out = out_dir / f"{prefix}_{slug}.vZome"
        ...
        manifest.append(
            {
                "signature": sig,
                "file": str(out),
                "polytope": f"{args.n}-demicube",
                "gosset_symbol": prefix,
