"""Emit vZome files for 1_22 / E6-root-polytope sweep hits."""
...
spec = importlib.util.spec_from_file_location("sweep122", Path(__file__).resolve().parent / "zphi_1_22_sweep.py")
sweep122 = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(sweep122)
...
def emit_hit(sig, hit, out_path):
    ...
    roots = sweep122.build_e6_roots()
    edges = e6_edges(roots)
...
def main():
    ...
    ap.add_argument("--json", default="ongoing_work/gosset_1_22_gauge/column_sweep_1_22_R3.json")
    ap.add_argument("--out_dir", default="output/gosset_1_22_candidates")
    args = ap.parse_args()
    d = json.loads((ROOT / args.json).read_text())
    out_dir = ROOT / args.out_dir
    manifest = []
    for sig, hit in sorted(d["hits"].items()):
        out = out_dir / f"1_22_{sig}.vZome"
