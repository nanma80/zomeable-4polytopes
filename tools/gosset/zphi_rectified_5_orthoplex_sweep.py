"""Raw-column Z[phi]^3 sweep for the rectified 5-orthoplex.

The rectified 5-orthoplex is the D5 root polytope with vertices all
permutations of (+/-1, +/-1, 0, 0, 0). Its edge directions are the D5 roots
+/-e_i +/- e_j, so the same column-compatibility condition used for the
5-demicube applies:

    c_i + c_j and c_i - c_j must be zome-axis/zero.

This script performs an independent sweep and evaluates the rectified
5-orthoplex source directly; it does not read the completed 1_21 hit file.
"""
...
def build_rectified_5_orthoplex():
    verts = []
    for i in range(5):
...
def dfs_sweep(R: int, out_dir: Path, max_seconds: float | None = None):
    V, E = build_rectified_5_orthoplex()
    cols = col.make_columns(R, include_zero=True)
    print(f"rectified 5-orthoplex column sweep R={R}")
    ...
    payload = {
        "polytope": "rectified_5_orthoplex",
        "description": "rectified 5-orthoplex / D5 root polytope / t1 2_11",
        "R": R,
        "status": status,
    ...
    out_file = out_dir / f"column_sweep_rectified_5_orthoplex_R{R}.json"
...
def main():
    ...
    ap.add_argument("--out_dir", default="ongoing_work/gosset_rectified_5_orthoplex")
