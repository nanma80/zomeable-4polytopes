"""Batch runner for approximate Archimedean zome searches."""
...
def branch_cap_for(target: str) -> int:
    if target == "truncated_icosidodecahedron":
        return 64
    if target in {"truncated_cuboctahedron", "truncated_icosahedron", "truncated_dodecahedron", "rhombicosidodecahedron"}:
        return 128
    return 320
...
def main() -> None:
    ...
    parser.add_argument("--targets", nargs="+", default=NON_SNUB_ARCHIMEDEAN_TARGETS)
    ...
    out_root = args.out or script_dir / f"output_archimedean_batch_{stamp}"
    out_root.mkdir(parents=True, exist_ok=True)
    log_dir = out_root / "_logs"
    log_dir.mkdir(exist_ok=True)

    batch = {
        "purpose": "Automated non-snub Archimedean approximate zome searches.",
        "started": datetime.now().isoformat(timespec="seconds"),
        "params": vars(args) | {"out": str(out_root)},
        "targets": [],
    }
