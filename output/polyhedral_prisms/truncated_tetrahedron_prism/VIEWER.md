# truncated tetrahedron prism — interactive 3D viewer

➡️ **[Open this page on GitHub Pages](https://nanma80.github.io/zomeable-4polytopes/output/polyhedral_prisms/truncated_tetrahedron_prism/VIEWER.html)** to interact with the 2 models below.

For methodology, kernel directions, search subtleties, and reproduction commands, see [`RESULTS.md`](RESULTS.md) in the same folder.

<script type='module' src='https://www.vzome.com/modules/vzome-viewer.js'></script>


<style>
  .vzome-card {
    box-sizing: border-box;
    border: 1px solid #ddd;
    border-radius: 0.5rem;
    padding: 0.25rem;
    background: #fff;
    max-width: min(100%, 72dvh);
    margin: 2rem auto;
    width: 100%;
  }
  .vzome-card vzome-viewer {
    display: block;
    width: 100%;
    aspect-ratio: 1 / 1;
    height: auto;
  }
  .vzome-card figcaption {
    margin: 0.75rem 0 0.5rem;
    color: #555;
    text-align: center;
    font-style: italic;
  }
</style>

<figure class="vzome-card">
 <vzome-viewer src="cell_first_truncated_tetrahedron.vZome" progress="true" >
 </vzome-viewer>
 <figcaption>
    cell_first_truncated_tetrahedron.vZome
 </figcaption>
</figure>

<figure class="vzome-card">
 <vzome-viewer src="face_first_square.vZome" progress="true" >
 </vzome-viewer>
 <figcaption>
    face_first_square.vZome
 </figcaption>
</figure>

<figure class="vzome-card">
 <vzome-viewer src="../truncated_tetrahedron_prism_hidden_cells/face_first_square_front_visible.vZome" progress="true" >
 </vzome-viewer>
 <figcaption>
    face_first_square.vZome — hidden-cell-removal view
 </figcaption>
</figure>
